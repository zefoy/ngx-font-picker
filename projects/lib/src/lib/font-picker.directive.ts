...
export class FontPickerDirective implements OnInit, OnChanges {
  private dialog: FontPickerComponent;
  ...
  @Input("fpPositionRelativeToArrow") fpPositionRelativeToArrow: boolean =
    false;
  ...
  public toggleDialog(): void {
    if (!this.dialog) {
      ...
      if (this.fpUseRootViewContainer && this.fpDialogDisplay !== "inline") {
        ...
        if (vcRef === this.vcRef) {
          console.warn(
            "You are using fpUseRootViewContainer, " +
      ...
      const compFactory =
        this.resolver.resolveComponentFactory(FontPickerComponent);
      const injector = Injector.create({
        providers: [],
        parent: vcRef.injector,
      });

      this.dialog = vcRef.createComponent(
        compFactory,
        0,
        injector,
        []
